...
void INA226value::enable() {
  app.onRepeat(read_delay, [this](){
      switch (val_type) { 
        case bus_voltage: output = pINA226->readBusVoltage(); // Volts
                break;
        case shunt_voltage: output = (pINA226->readShuntVoltage() / 1000); // Volts
                break;
        case current: output = (pINA226->readShuntCurrent() / 1000); // Amps
                break;
        case power: output = (pINA226->readBusPower() / 1000); // Watts
                break; 
        case load_voltage: output = pINA226->readBusVoltage() + pINA226->readShuntVoltage(); // Volts
                 break; 
